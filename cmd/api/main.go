...
func main() {
	...
	// нотифаер апи: обновляет кэш турниров и лидерборда, а события для вебсокета
	// публикует в редис. локально не рассылает: клиенты турнира могут сидеть
	// на другой реплике, до всех (и до этой) событие дойдёт через мост ниже
	notifier := &events.SyncNotifier{
		TournamentCache: tournamentCache,
		Leaderboard:     leaderboardCache,
		Redis:           events.NewRedisEventPublisher(redisCache, log),
		Log:             log,
	}

	// Мост Redis Pub/Sub: принимает события воркера и реплик API для рассылки по WebSocket.
	// у отдельного нотифаера только broadcaster - события должны только рассылаться,
	// а не повторно дёргать кэш (отправитель уже обновил общий)
