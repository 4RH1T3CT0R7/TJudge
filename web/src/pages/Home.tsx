...
// Prisoner's Dilemma Matrix Component - Full width centered design
  ...
  return (
    <div className="w-full flex flex-col items-start py-4 pl-4">
      ...
      <div className="flex items-center">
        {/* Player A label */}
        <div className="flex items-center justify-center w-10 h-48">
          <div
        ...
        {/* Row labels + Matrix grid with Player B on top */}
        <div className="flex flex-col">
          {/* Player B label - centered over the matrix cells only */}
          <div className="flex">
            <div className="w-14" /> {/* Spacer for row labels */}
            <div className="flex-1 text-center text-lg font-bold text-gray-800 dark:text-gray-200 mb-2" style={{ width: '224px' }}>
              Игрок B
            </div>
          </div>
          ...
          <div className="flex mb-1">
            <div className="w-14" /> {/* Spacer for row labels */}
            <div className="w-28 text-center text-sm font-semibold text-emerald-600 dark:text-emerald-400">Сотр.</div>
            <div className="w-28 text-center text-sm font-semibold text-rose-600 dark:text-rose-400">Пред.</div>
          </div>

          {/* Row 1: Cooperate */}
          <div className="flex items-center">
            <div className="w-14 text-right pr-2 text-sm font-semibold text-emerald-600 dark:text-emerald-400">Сотр.</div>
            <div
              className={`w-28 h-20 flex items-center justify-center cursor-pointer transition-all rounded-tl-xl ${hoveredCell === 'cc' ? 'scale-105 shadow-2xl z-10' : 'hover:brightness-110'}`}
            ...
            <div
              className={`w-28 h-20 flex items-center justify-center cursor-pointer transition-all rounded-tr-xl ${hoveredCell === 'cd' ? 'scale-105 shadow-2xl z-10' : 'hover:brightness-110'}`}
          ...
          {/* Row 2: Defect */}
          <div className="flex items-center">
            <div className="w-14 text-right pr-2 text-sm font-semibold text-rose-600 dark:text-rose-400">Пред.</div>
            <div
              className={`w-28 h-20 flex items-center justify-center cursor-pointer transition-all rounded-bl-xl ${hoveredCell === 'dc' ? 'scale-105 shadow-2xl z-10' : 'hover:brightness-110'}`}
              style={{ backgroundColor: '#dc2626' }}
            ...
            <div
              className={`w-28 h-20 flex items-center justify-center cursor-pointer transition-all rounded-br-xl relative ${hoveredCell === 'dd' ? 'scale-105 shadow-2xl z-10' : 'hover:brightness-110'}`}
              style={{ backgroundColor: '#ca8a04' }}
              onMouseEnter={() => setHoveredCell('dd')}
              onMouseLeave={() => setHoveredCell(null)}
            >
              <span className="font-mono font-bold text-xl text-white">1, 1</span>
              <div className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-cyan-400 rounded-full" title="Равновесие Нэша" />
            </div>
          </div>

          {/* Tooltip */}
          <div className={`mt-4 transition-all duration-200 h-10 ${hoveredCell ? 'opacity-100' : 'opacity-50'}`}>
            {hoveredCell ? (
              <>
