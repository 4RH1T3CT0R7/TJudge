...
// Metrics - все прометеевские метрики приложения в одном месте
type Metrics struct {
	// матчи
...
// New отдаёт singleton. именно singleton - promauto регистрирует метрики
// в дефолтном регистре, второй раз тот же Name = паника duplicate registration,
// а New() зовут из обоих процессов и кучи тестов
func New() *Metrics {
	once.Do(func() {
		instance = &Metrics{
...
func (m *Metrics) RecordMatchComplete(gameType string, status string, duration time.Duration) {
	...
	// осторожно: тут аргументы (gameType, status), а лейблы (status, game_type) -
	// порядок в WithLabelValues обратный сигнатуре, так исторически, не путать
...
// PrimeCacheType заводит нулевые серии hit/miss, чтобы дашборды до первого
// трафика показывали нули а не no data
...
// RecordQueueDeadLetterPush - reason короткая метка причины (unmarshal_error и т.п.)
