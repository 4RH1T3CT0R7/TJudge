...
func (h *BroadcastHandler) Handle(_ context.Context, event any) error {
	...
	case events.TournamentStarted:
		h.broadcaster.Broadcast(e.TournamentID, "tournament_update", map[string]interface{}{
			"status":     e.Status,
	...
	case events.MatchesCreated:
		h.broadcaster.Broadcast(e.TournamentID, "matches_created", map[string]interface{}{
			"program_id":    e.ProgramID.String(),
			"matches_count": e.MatchCount,
		})

	case events.MatchResultProcessed:
		h.broadcaster.Broadcast(e.TournamentID, "match_result", map[string]interface{}{
