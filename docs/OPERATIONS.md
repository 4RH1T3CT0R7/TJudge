...
Откат — на текущем checkout: `git checkout` тега, выпущенного до перехода на каталог программ хоста, вернёт volume `programs_data` со старыми файлами. `migrate` остаётся текущим (образа `tjudge-migrate` у ранних версий нет), down-миграции не нужны. Образы песочниц той версии скачиваются заранее: worker тянет их без авторизации ghcr.

```bash
V=<прошлая версия>
docker pull ghcr.io/4rh1t3ct0r7/tjudge-executor:$V && docker pull ghcr.io/4rh1t3ct0r7/tjudge-builder:$V
VERSION=$V P up -d --no-deps api worker
```
