...
| 4 типа игр | Дилемма заключённого, Справедливое предложение, Перетягивание каната, Баланс вселенной |
| Безопасное выполнение | Изолированные Docker-контейнеры |
| Real-time обновления | WebSocket для таблицы лидеров |
| Автоскейлинг | До 1000 параллельных матчей |
...
5. Загрузите свою программу-стратегию
...
- Фильтрация по программе, статусу, дате
...
2. Заполните форму:
   ...
   - **Тип игры** — выберите из созданных игр
...
### 2. Справедливое предложение (Good Deal)

**Идентификатор:** `good_deal`

#### Правила

- Награда: R = 100 очков
- Штраф: T = 50 очков
- Игроки чередуются ролями A (предлагающий) и B (решающий)

#### Механика

1. Игрок A предлагает разделение: число от 0 до R
2. Игрок B решает:
   - `1` — принять (A получает предложенное, B получает остаток)
   - `0` — отклонить (оба получают 0)

#### Протокол взаимодействия

**Роль A (предлагающий):**
- Ввод: `A`
- Вывод: число от 0 до 100

**Роль B (решающий):**
- Ввод: `B <предложение>`
- Вывод: `1` (принять) или `0` (отклонить)
...
def main():
    ...
    if line == "A":
        # Роль предлагающего — предлагаем честное деление
        print(50)
    else:
        # Роль решающего
        _, offer = line.split()
        ...
        # Принимаем, если нам достаётся >= 30
        if 100 - offer >= 30:
            print(1)
        else:
            print(0)
...
### 3. Перетягивание каната (Tug of War)
...
### 4. Баланс Вселенной (Balance of Universe)

**Идентификатор:** `balance_of_universe`

#### Правила

- Бюджет: n = 100 единиц
- Игроки одновременно выбирают, сколько потратить
- Выплаты зависят от разницы

#### Расчёт очков

| Условие | Игрок 1 получает | Игрок 2 получает |
|---------|------------------|------------------|
| \|a - b\| ≤ 1 | a + b | a + b |
| a > b | a + b | 0 |
| b > a | 0 | a + b |
...
**Ввод:**
```
<ваш_бюджет> <бюджет_противника>
```

**Вывод:**
- Число от 0 до вашего бюджета
...
def main():
    line = input().strip()
    my_budget, opponent_budget = map(int, line.split())

    if my_budget == 0:
        print(0)
    else:
        # Случайная стратегия с небольшим перевесом
        choice = random.randint(1, min(my_budget, opponent_budget + 2))
        print(choice)
...
| Аргумент | Описание |
|----------|----------|
| `GAME_TYPE` | Тип игры: `prisoners_dilemma`, `good_deal`, `tug_of_war`, `balance_of_universe` |
...
# Создание турнира (требуется auth)
